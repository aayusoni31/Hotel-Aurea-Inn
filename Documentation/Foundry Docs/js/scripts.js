...
$(document).ready(function () {
  ...
  if (!$("nav").hasClass("fixed") && !$("nav").hasClass("absolute")) {
    ...
    if ($(window).width() > 768) {
      $(".parallax:first-child .background-image-holder").css(
        "top",
    ...
    if ($(window).width() > 768) {
      $("section.fullscreen:first-child").css(
        "height",
  ...
  $(".tweets-feed")
    ...
    .each(function (index) {
      ...
      twitterFetcher.fetch(
        $("#tweets-" + index).attr("data-widget-id"),
        "",
        5,
        true,
        true,
  ...
  $(".modal-video-container").each(function (index) {
    ...
    $(this)
      ...
      .clone()
      .appendTo("body")
      .attr("video-link", index);
  });

  $(".modal-video-container .play-button").click(function () {
    var linkedVideo = $("body").find(
      '.modal-video[video-link="' + $(this).attr("video-link") + '"]'
    );
...
$(window).load(function () {
  ...
  mr_firstSectionHeight = $(".main-container section:first-child").outerHeight(
    true
  );
}); //variant-remove
